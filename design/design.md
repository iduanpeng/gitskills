...
## 设计原则
1. 开闭原则：一个软件实体如类、模块和函数应该对扩展开放，对修改关闭，用抽象构建框架、用实现扩展细节。提高软件系统和可复用性和可维护性
2. 依赖倒置原则：高层模块不应该依赖低层模块，二者都应该依赖其抽象，抽象不应该依赖细节；细节应该依赖抽象，针对接口编程，不要针对实现编程。可以减少类间的耦合性，提高代码可读性和可维护性
3. 
